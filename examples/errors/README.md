...
`RejectReason` is a small `enum`, one variant per rejection reason, each
mapped to a stable code and message by two `match`-based methods:
...
impl RejectReason {
    fn code(self) -> i64 { /* -101..=-104, one per variant */ }
    fn message(self) -> &'static [u8] { /* one fixed string per variant */ }
    fn rollback(self) -> ! { rollback!(self.message(), self.code()) }
}
```
...
```sh
cargo run -p hooks-build -- build --manifest-path examples/errors/Cargo.toml
```
