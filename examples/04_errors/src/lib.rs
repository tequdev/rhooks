...
//! system: a small [`hooks_lib::hook_errors!`] enum of rejection reasons,
//! each with its own negative `i64` code, matched to a distinct
//! `rollback!` call. See the README for the full code table and how it
//! shows up as `HookExecution.HookReturnCode` in transaction metadata.
//!
//! Build: `hooks-build build --manifest-path examples/04_errors/Cargo.toml`
...
hook_errors! {
    ...
    /// range (`-1..=-45`, plus `-10024`) so a `HookReturnCode` of, say,
    /// `-101` is unambiguously *this hook's* policy, not a Hook API
    /// failure — see the README's code table.
    pub enum RejectReason {
        /// Couldn't read the originating transaction's `Account` field at
        /// all (should not happen for a well-formed transaction;
        /// defensive).
        BadAccountField = -101,
        /// `SourceTag` matches [`BLOCKED_SOURCE_TAG`].
        BlockedSourceTag = -102,
        /// `Amount` isn't a native (XRP/XAH) amount — this hook's policy
        /// only understands native amounts (see `examples/07_xfl-math` for
        /// handling IOU amounts too).
        NotNativeAmount = -103,
...
impl RejectReason {
    ...
    /// Roll the hook back with this reason's message and code (`code()`,
    /// generated by [`hooks_lib::hook_errors!`]). Never returns (see
    /// [`hooks_lib::api::control::rollback`]).
    fn rollback(self) -> ! {
