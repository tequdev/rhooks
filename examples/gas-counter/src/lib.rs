...
//! make.
//!
//! Build: `hooks-build build --manifest-path examples/gas-counter/Cargo.toml --api-version 1`
...
/// A fixed step table, summed with an ordinary `for` loop in [`hook`] below.
...
pub extern "C" fn hook(_reserved: u32) -> i64 {
    ...
    if state_set(&next.to_le_bytes(), &STATE_KEY).is_err() {
        rollback!(b"gas-counter: state_set failed", -1);
    }
