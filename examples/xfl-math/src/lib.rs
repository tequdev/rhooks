...
//! assuming success.
//!
//! Build: `hooks-build build --manifest-path examples/xfl-math/Cargo.toml`
...
pub extern "C" fn hook(_reserved: u32) -> i64 {
    ...
    let txn_slot = match otxn_slot(0) {
        Ok(s) => s,
        Err(_) => rollback!(b"xfl-math: otxn_slot failed", -1),
    };
    let amount_slot = match slot_subfield(txn_slot, sfAmount, 0) {
        Ok(s) => s,
        Err(_) => rollback!(b"xfl-math: no Amount field on otxn", -1),
    };
    ...
    let amount = match XFL::from_slot(amount_slot) {
        ...
        Err(_) => rollback!(b"xfl-math: Amount is not a valid XFL amount", -1),
    };
    ...
    let share = match amount.mulratio(false, PERCENT_NUM, PERCENT_DEN) {
        ...
        Err(_) => rollback!(b"xfl-math: mulratio failed (overflow?)", -1),
    };
    ...
    let min_share = match XFL::new(-21, 1_000_000_000_000_000) {
        Ok(x) => x,
        Err(_) => rollback!(b"xfl-math: could not construct min_share", -1),
    };
    ...
    match share.lt(min_share) {
        Ok(true) => rollback!(b"xfl-math: computed share below minimum", -1),
        Ok(false) => {}
        Err(_) => rollback!(b"xfl-math: comparison failed", -1),
    }
