...
//!
//! Build: `hooks-build build --manifest-path examples/guard-patterns/Cargo.toml`

#![no_std]

use hooks_lib::prelude::*;
use hooks_lib::{accept, guard, guard_m, rollback};
...
/// (`examples/firewall`) does exactly this and, as a result, needs
...
pub extern "C" fn hook(_reserved: u32) -> i64 {
    ...
    match otxn_field(&mut sender, sfAccount) {
        ...
        _ => rollback!(b"guard-patterns: could not read otxn sender", -1),
    }
    ...
    if accounts_equal(&sender, &blocked) {
        rollback!(b"guard-patterns: blocked account", -1);
    }
