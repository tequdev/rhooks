...
4. How a loop's `maxiter` shows up in `hooks-build`'s reported worst-case
...
own source. `firewall` (`examples/firewall`) does exactly this, and as a
result needs:
...
`maxiter` is exact — `hooks-build build` needs **no extra flags at all**
for this function.
...
code — e.g. a macro like `hooks_lib::txn_template!` that expands to more
...
calls above so both loops share id `1` still passes `hooks-build build`
...
|---|---:|---:|
| `accounts_equal` only (the two `guard_m!` loops removed) | 779 | 529 bytes |
| Full hook (`accounts_equal` + both `guard_m!` loops) | **1282** | **644 bytes** |

The two `maxiter = 8` demonstration loops together add **503** instructions
to the worst case (`1282 - 779`) — a mechanical illustration of the point
...
is precisely what "worst case" means). A loop with a needlessly large
`maxiter` doesn't just risk hiding real bugs — it also directly and
proportionally inflates the number SetHook's fee estimation is based on.

## Build

```sh
cargo run -p hooks-build -- build --manifest-path examples/guard-patterns/Cargo.toml
```
...
- `BL` configured and matches the otxn sender → rollback
  (`"guard-patterns: blocked account"`).
