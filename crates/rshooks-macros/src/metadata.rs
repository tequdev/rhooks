...
//! a distinctive wasm export name, and extracted by `rshooks-build` before its
...
pub(crate) fn expand(input: TokenStream) -> TokenStream {
    ...
    for variant in variants(&metadata) {
        generated.push_str(&format!(
            "const _: ::rshooks::tx_type::TxType = \
             ::rshooks::tx_type::TxType::{};",
            variant.name
        ));
    }
    generated.push_str(&format!(
    ...
    generated.parse::<TokenStream>().unwrap_or_else(|_| {
        err(
            Span::call_site(),
            "rshooks-macros: internal metadata! expansion failed",
        )
    })
}
