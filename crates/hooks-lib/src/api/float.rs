...
pub fn float_sto_set(buf: &[u8]) -> Result<XFL> {
    res(unsafe { hooks_core::float_sto_set(buf.as_ptr() as u32, buf.len() as u32) })
...
pub fn slot_float(slot_no: u32) -> Result<XFL> {
    res(unsafe { hooks_core::slot_float(slot_no) }).map(XFL::from_raw_bits)
}
