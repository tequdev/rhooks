...
pub fn trace(msg: &[u8], data: &[u8], as_hex: bool) -> Result<i64> {
    ...
    {
        if let Some(v) = hooks_core::backend::with_backend(|b| b.trace(msg, data, as_hex)) {
            return Ok(v);
        }
    }
    res(unsafe {
...
pub fn trace_num(msg: &[u8], number: i64) -> Result<i64> {
    #[cfg(all(feature = "testenv", not(target_arch = "wasm32")))]
    {
        if let Some(v) = hooks_core::backend::with_backend(|b| b.trace_num(msg, number)) {
            return Ok(v);
        }
    }
    res(unsafe { hooks_core::trace_num(msg.as_ptr() as u32, msg.len() as u32, number) })
}

/// Emit a trace message (`msg`) followed by an XFL `value`.
#[inline(always)]
pub fn trace_float(msg: &[u8], value: XFL) -> Result<i64> {
    res(unsafe { hooks_core::trace_float(msg.as_ptr() as u32, msg.len() as u32, value.raw_bits()) })
}

/// Internal support for the `trace!`/`trace_num!`/`trace_float!` macros.
///
/// The feature gate must be evaluated inside hooks-lib (a `#[cfg]` written
...
/// always-present shims, which compile to no-ops unless hooks-lib's own
