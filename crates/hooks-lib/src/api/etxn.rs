...
use crate::types::{EMIT_DETAILS_LEN, EmitDetails, HASH_LEN, Hash, NONCE_LEN, Nonce};
...
/// The serialized `EmitDetails` structure for the next transaction this hook
/// would emit.
#[inline(always)]
pub fn etxn_details() -> Result<EmitDetails> {
    let mut buf: EmitDetails = [0u8; EMIT_DETAILS_LEN];
    res(unsafe { hooks_core::etxn_details(buf.as_mut_ptr() as u32, buf.len() as u32) })?;
    Ok(buf)
}
...
mod tests {
    ...
    fn smoke_not_implemented_on_host() {
        ...
        assert_eq!(etxn_details(), Err(HookError::NotImplemented));
