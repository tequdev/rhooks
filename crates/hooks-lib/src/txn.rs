...
//!
//! `hooks-lib` does not hard-code a `PaymentTemplate` (or any other
//! transaction-shaped type): any new field or transaction type a hook author
//! wanted would require a `hooks-lib` release. Instead, mirroring xahaud's
...
/// // hooks-lib test suite for the byte-compat and required-field
/// // regression proofs).
...
mod tests {
    ...
    fn prepare_for_emit_propagates_host_stub_errors() {
        ...
        // `NOT_IMPLEMENTED` stub (see hooks-core), so `prepare_for_emit`
