...
//! explicit paths (`hooks_lib::api::slot::slot_clear`,
//! `hooks_lib::api::otxn::otxn_slot`) so that mixing them is at least always
...
use crate::types::{AccountId, CurrencyCode, Hash, IouAmount, Keylet, NativeAmount};
use crate::xfl::XFL;
...
/// Marker: the slot holds an `STObject` — a ledger object, a transaction, or
/// any nested object field. Navigable by [`SField`]; has no `value()` of its
/// own (read its fields, or use the raw byte escapes).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct STObject;

/// Marker: the slot holds an `STArray`. Navigable by `u32` index, and
/// [`count`](SlotObject::count) reports its length.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct STArray;

/// Marker: the slot holds a serialized `Amount` — 8 bytes native or 48 bytes
/// IOU. See [`SlotObject::as_xfl`] and [`SlotObject::value`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Amount;

/// Marker: the slot holds a serialized `Issue` — 20 bytes native or 40 bytes
/// IOU (currency + issuer).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Issue;

/// Marker: the slot's contents have no typed read in this layer — a `Blob`,
/// a `PathSet`, a `Hash160` (whose fields mean different things), or a slot
/// whose type was never established.
///
/// Still fully usable: navigable by either key kind (the host decides at
/// runtime whether the operation makes sense) and readable through
/// [`raw`](SlotObject::raw)/[`raw_exact`](SlotObject::raw_exact).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Opaque;
...
impl<T> SField<T> {
    ...
    /// [`SlotObject::assume_type`] escape. Reading a field as something it
...
/// # Path length
///
/// The expansion nests one `match` per hop, but `hooks-build`'s unnest pass
