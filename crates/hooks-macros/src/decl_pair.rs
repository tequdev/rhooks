...
/// state side forwards [`StateKeyEncode::encode`](::hooks_lib::state::StateKeyEncode::encode)
...
fn pairing_entity_impls(entity: &str, key_ty: &str, value: &str, role: Role) -> String {
    match role {
        Role::State => format!(
            "
#[automatically_derived]
impl ::hooks_lib::state::StateKeyEncode for {entity} {{
    #[inline(always)]
    fn encode(&self) -> ::hooks_lib::state::EncodedStateKey {{
        <{key_ty} as ::hooks_lib::state::StateKeyEncode>::encode(&self.0)
    }}
}}
{pairing}
",
        ...
        Role::HookParam | Role::OtxnParam => format!(
            "
#[automatically_derived]
impl ::hooks_lib::convert::ToBytes for {entity} {{
    const MAX_LEN: usize = <{key_ty} as ::hooks_lib::convert::ToBytes>::MAX_LEN;

    #[inline(always)]
    fn write(&self, buf: &mut [u8]) -> usize {{
        ::hooks_lib::convert::ToBytes::write(&self.0, buf)
    }}
}}

{length_assert}
#[automatically_derived]
impl ::hooks_lib::convert::TypedParamName for {entity} {{
    type Value = {value};

    #[inline(always)]
    fn with_name_bytes<__R>(&self, f: impl ::core::ops::FnOnce(&[u8]) -> __R) -> __R {{
        <{key_ty} as ::hooks_lib::convert::TypedParamName>::with_name_bytes(&self.0, f)
    }}
}}
",
