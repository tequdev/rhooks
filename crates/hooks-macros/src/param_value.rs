//! `#[derive(ParamValue)]` — backs `hooks_lib::ParamValue`.
//!
...
//!
//! See `hooks_lib::HookData`'s/`hooks_lib::ParamName`'s doc comments for
...
",
    ...
    match src.parse::<TokenStream>() {
        ...
        Err(_) => err(
            shape.name_span,
            "hooks-macros: internal ParamValue codegen failed to parse",
        ),
    }
}
