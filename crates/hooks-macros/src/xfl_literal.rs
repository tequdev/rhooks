...
//! `hooks_lib::xfl::XFL::from_raw_bits` expects.
//!
...
//!
//! See `hooks_lib::XFL!`'s doc comment (the public re-export site) for the
...
/// 97 bias) -- mirrors `hooks_lib::xfl`'s module doc comment bit layout.
...
/// `XFL!`'s `#[proc_macro]` entry point (see `hooks_macros::XFL`, the thin
...
pub(crate) fn expand(input: TokenStream) -> TokenStream {
    ...
    let src = format!("::hooks_lib::xfl::XFL::from_raw_bits({bits}i64)");
    src.parse::<TokenStream>()
        .unwrap_or_else(|_| err(span, "hooks-macros: internal XFL! expansion failed"))
}
