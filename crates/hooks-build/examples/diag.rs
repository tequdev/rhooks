...
fn main() {
    ...
    let opts = hooks_build::Options::default();
    let cleaned = hooks_build::clean(&wasm, &opts).unwrap();
    fs::write("/tmp/diag_cleaned.wasm", &cleaned).unwrap();
    let (flattened, freport) = hooks_build::flatten(&cleaned).unwrap();
    for n in &freport.notes {
        eprintln!("flatten note: {n}");
    }
    fs::write("/tmp/diag_flattened.wasm", &flattened).unwrap();
    let (unnested, ureport) = hooks_build::unnest(&flattened).unwrap();
