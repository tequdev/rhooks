...
use hooks_build::metadata::{
    HookMetadata, METADATA_EXPORT_PREFIX, build_metadata, extract_metadata, hook_hash,
};
use hooks_build::{ApiVersion, GuardVerdict, Options, ValidationReport};
...
fn carrier_has_no_effect_on_cleaned_wasm() {
    ...
    let (baseline, _) =
        hooks_build::run_pipeline(&baseline, &opts).expect("baseline pipeline succeeds");
    let (carried, _) =
        hooks_build::run_pipeline(&carried, &opts).expect("carrier pipeline succeeds");
