...
fn validator_hints_buf_eq_for_compare_like_loop() {
    let err = hooks_build::validate(&wasm(COMPARE_LIKE_LOOP_HOOK), &opts()).unwrap_err();
    let msg = err.to_string();
    assert!(
    ...
    assert!(
        msg.contains("buf_eq"),
        "hint should point at hooks_lib::buf_eq_<N>: {msg}"
    );
}

#[test]
fn validator_hints_static_for_copy_like_loop() {
    let err = hooks_build::validate(&wasm(COPY_LIKE_LOOP_HOOK), &opts()).unwrap_err();
    let msg = err.to_string();
    assert!(
...
fn validator_hints_static_for_zero_init_like_loop() {
    let err = hooks_build::validate(&wasm(ZERO_INIT_LIKE_LOOP_HOOK), &opts()).unwrap_err();
    let msg = err.to_string();
    assert!(
    ...
    assert!(
        ...
        "hint should point at hooks_lib::static_cell::HookStatic: {msg}"
    );
}

#[test]
fn validator_suppresses_hint_when_loop_calls_an_import() {
    let err = hooks_build::validate(&wasm(CALLING_LOOP_HOOK), &opts()).unwrap_err();
    let msg = err.to_string();
    assert!(
        msg.to_lowercase().contains("guard"),
