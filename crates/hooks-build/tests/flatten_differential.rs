...
//! `hooks-build` binary), with the same stubbed `env` import. An inlining
...
fn assert_differential(wat: &str, cases: &[(&str, i32)]) -> (Vec<u8>, hooks_build::FlattenReport) {
    let pre = wat::parse_str(wat).expect("fixture is valid wat");
    let (post, report) = hooks_build::flatten(&pre).expect("flatten succeeds");
